...
/** Generates a content ID for a plain text node within a container. */
internal fun contentId(parent: Container): String =
...
object InkParser {
    ...
    fun parse(input: String, provider: StoryWrapper, fileName: String): Story {
        ...
        val content = mutableMapOf<String, InkObject>()
    ...
    private fun parseLine(lineNumber: Int, line: String, currentContainer: Container?): List<InkObject> {
        ...
        when (firstChar) {
            ...
            Symbol.CBRACE_RIGHT -> if (currentContainer is Conditional || currentContainer is ConditionalOption)
                return mutableListOf(getConditional(currentContainer).parent as InkObject)
            else -> { }
        }
        if (line.contains(Symbol.DIVERT))
            return parseDivert(lineNumber, line, currentContainer)
        if (line.isNotEmpty() && currentContainer != null)
            return parseContainer(InkObject().apply {
                id = contentId(currentContainer)
                text = line
    ...
    private fun parseContainer(cont: InkObject): MutableList<InkObject> {
    ...
    internal fun parseDivert(lineNumber: Int, line: String, currentContainer: Container?): List<InkObject> {
        val ret = mutableListOf<InkObject>()
        val div = line.split(Symbol.DIVERT)
        if (div[0].isNotEmpty()) {
            val text = InkObject().apply {
                id = contentId(currentContainer!!)
                this.text = "${div[0]}${Symbol.GLUE}"
                parent = currentContainer
                this.lineNumber = lineNumber
            }
