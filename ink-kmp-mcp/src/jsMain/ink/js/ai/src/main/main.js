...
const McpLlmClient = require('./mcpLlmClient');
...
class InkAIAssistant {
  constructor() {
    ...
    this.llm = null; // Initialized in init() — MCP or direct OpenAI
   ...
   * Tries MCP server first — if reachable, uses McpLlmClient (server-managed models).
   * Falls back to direct LLMClient (local Ollama/LM Studio via OpenAI SDK).
   */
  async checkDependencies() {
    ...
    // Try MCP server first — provides server-managed LLM (JLama, LM Studio, cloud)
    const mcpClient = new McpLlmClient(this.mcpServerUrl);
    const mcpAvailable = await mcpClient.isAvailable();
    if (mcpAvailable) {
      this.llm = mcpClient;
      console.log(`AI backend: MCP server at ${this.mcpServerUrl}`);
    } else {
      // Fall back to direct OpenAI-compatible API (Ollama, LM Studio)
      this.llm = new LLMClient();
      console.log('AI backend: Direct OpenAI-compatible API (Ollama/LM Studio)');
      ...
      if (!llmAvailable) {
        dialog.showMessageBox(this.mainWindow, {
          ...
          message: 'Could not connect to MCP server or local LLM.\n\n' +
            `MCP server: ${this.mcpServerUrl} (not reachable)\n` +
            'Local LLM: Ollama/LM Studio (not running)\n\n' +
            'Start the MCP server (gradle runServer) or a local LLM to enable AI features.',
          buttons: ['OK'],
        });
      }
    }
