 ...
 */
import { parseMcpResponseSafe } from './lib/mcp-helpers.js';
...
function handleStory(resp) {
  const { data, error: parseErr } = parseMcpResponseSafe(resp);
  if (parseErr) {
    $('#storyText').innerHTML = `<p style="color:red">${parseErr}</p>`;
    return;
  }
