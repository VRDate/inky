...
class UnicodeSymbolParser {
    ...
    private val log = LoggerFactory.getLogger(UnicodeSymbolParser::class.java)
    ...
    fun parseEmojiTest(lines: List<String>): ParseResult {
        ...
        log.debug("Parsed {} emoji entries across {} groups", entries.size, groups.size)
    ...
    private fun parseEmojiTestLine(line: String, group: String, subgroup: String): UnicodeEntry? {
        ...
        val symbol = codePoints.joinToString("") { String(Character.toChars(it)) }
    ...
    fun parseUnicodeData(
        ...
        log.debug("Parsed {} symbol entries from {} blocks", entries.size, blocks.size)
        return ParseResult(entries, groups)
    }
