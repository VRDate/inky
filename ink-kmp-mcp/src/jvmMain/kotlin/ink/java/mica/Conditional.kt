...
import ink.java.mica.util.InkParseException
...
internal class Conditional(
    ...
    parent: Container,
    lineNumber: Int
) : Container(getId(parent), "", parent, lineNumber) {
    ...
    init {
        val str = header.substring(1).trim()
        if (str.isNotEmpty()) {
            if (!str.endsWith(":"))
                throw InkParseException(
                    "Error in conditional block; initial condition not ended by ':'. Line number: $lineNumber"
                )
            val condition = str.substring(0, str.length - 1).trim()
    ...
    fun resolveConditional(story: Story): Container {
        ...
        when (seqType) {
            ...
            SequenceType.SEQUENCE_STOP -> return children[
                if (count >= children.size) children.size - 1 else count
            ] as Container
        }
    ...
    companion object {
        ...
        fun isConditionalHeader(str: String): Boolean =
            str.startsWith(Symbol.CBRACE_LEFT) && !str.contains(Symbol.CBRACE_RIGHT)
    }
}
