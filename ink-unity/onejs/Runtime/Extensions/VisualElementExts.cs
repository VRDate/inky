...
namespace OneJS.Extensions {
    public static class VisualElementExts {
        ...
        public static UQueryBuilder<VisualElement> Query(this VisualElement e, Type type, string name = null, params string[] classes) {
            ...
            var queryMethod = typeof(UQueryExtensions).GetMethod("Query", 1, new Type[] { typeof(VisualElement), typeof(string), typeof(string[]) }).MakeGenericMethod(type);

            var result = queryMethod.Invoke(null, new object[] { e, name, classes });

            var ofTypeMethod = result.GetType().GetMethod("OfType", 1, new Type[] { typeof(string), typeof(string[]) }).MakeGenericMethod(typeof(VisualElement));
